...
part2_version4 = function(t){
  ...
  for(k in 0:(dt - 1)){
    temp2 = 1
    for(i in (1+k): max(k+1, dt-1)){
      part3 = ni(i)
      if(part3 == 0){  part3 = 10e-10  }
      temp2 = temp2 * (1 - min(part3/(part3+3), rou(Xd[i])/part3))
    }
    temp = temp + ci(k) * temp2
  }
  i = dt
  ...
  finals = (1 - min(part3/(part3+3), rou(Xd[i])/part3))
  temp = temp / finals
  return(temp /N)
}
...
Spt = function(t){
  ...
  s4 = part1 + part2_version4(t)

  return(list(s1 = s1, s2 = s2, s3 = s3, s4 = s4))
}

# the effects of four methods
s1 = c(); s2 = c(); s3 = c(); s4=c(); s0 = c()
for(t in data$time){
  res = Spt(t)
  s0 = c(s0, S(t))
  s1 = c(s1, res$s1)
...
lines(data$time, s4, lty = 6, col = 5, cex = 2)
...
legend('topright', legend = c('True S(t)','Slud','change1','change2','change3','KM'),
       col = c(1,2,3,4,5,6),
       lty = c(1,2,5,4,6,1), cex = 0.5)
...
part2_version2 = function(t){
  ...
  for(k in 0:(dt - 1)){
    ...
    for(i in (1+k):dt){
      ...
      xd = Xd[i]
      if(Xd[i] == 0){xd = 10e-10}
      temp2 = temp2 * (1 - min(part3/(part3+1), rho(xd)/part3))
    }
    temp = temp + ci(k) * temp2
  }
  return(temp /N)
...
part2_version3 = function(t){
  ...
  finals = (1 - min(part3/(part3+3),rho(xd)/part3))
